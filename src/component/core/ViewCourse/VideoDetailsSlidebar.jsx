  ...
  return (
    <>
        <div className={`${showSidebar ? "" : "hidden"} w-6 h-72 md:hidden absolute center`}>
          <FaChevronRight onClick={() => {setShowSidebar (!showSidebar)}} className={`md;hidden z-10  cursor-pointer text-2xl aplaxtion text-richblack-900 m-2 bg-richblack-100 rounded-full p-1 top-3 absolute left-0`}/>
         ...
         <div className={`${showSidebar ? "h-0 w-0" : "h-[calc(100vh-3.5rem)] w-[320px]"} transition-all duration-700 z-20 relative offSidebar1`}>
            ...
            <div className='h-[calc(100vh - 5rem)] overflow-y-auto px-2'>
                {
                    courseSectionData?.mapp((section, index) =>(
                        <details key={index} 
